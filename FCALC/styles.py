import tkinter as tkinter
import tkinter.ttk as ttk



class FStyle(ttk.Style):
    '''Une class e Style ttk personnalisé :
        -   FStyle_RoundedFrame
    '''
    img1 = """
    """
    img2 = """
    """

    def __init__(self):
        ttk.Style.__init__(self)
        style_name = "FStyle_RoundedFrame"
        if style_name not in self.element_names():
            self.f_style_images = []
            self.f_style_images.append(tkinter.PhotoImage("frameFocusBorder", data=self.img1))
            self.f_style_images.append(tkinter.PhotoImage("frameBorder", data=self.img2))
            self.element_create(style_name, "image", "frameBorder",
                    ("focus", "frameFocusBorder"), border=2, sticky="nsew")
            self.layout(style_name, [(style_name, {"sticky": "nsew"})])
        
        


if __name__ == '__main__':
    '''Exemple
    '''
    root = tkinter.Tk()
    mes_styles = FStyle()
    frame = ttk.Frame(style="FStyle_RoundedFrame", padding=10)
    frame.pack(fill='x')
    frame2 = ttk.Frame(style="FStyle_RoundedFrame", padding=10)
    frame2.pack(fill='both', expand=1)
    entry = ttk.Entry(frame)
    entry.pack(fill='x')
    entry.bind("<FocusIn>", lambda evt: frame.state(["focus"]))
    entry.bind("<FocusOut>", lambda evt: frame.state(["!focus"]))
    text = tkinter.Text(frame2, borderwidth=0, bg="white", highlightthickness=0)
    text.pack(fill='both', expand=1)
    text.bind("<FocusIn>", lambda evt: frame2.state(["focus"]))
    text.bind("<FocusOut>", lambda evt: frame2.state(["!focus"]))
    root.mainloop()
